...
Your mental model isn't wrong — it just needs a nuance. 
Let me clarify when it works and when it doesn't.
Your mental model: O(choices^depth × cost_per_call)
This works for fixed branching problems where every node in the recursion
tree has the same number of children.
...
Depth: n
Result: nⁿ (or more precisely n! since used prunes, 
but the upper bound formula applies)
...
When each element is visited at most once per path and the decision 
is "include it or don't", think of it as:
...
Pattern: Fixed branching (same choices at every level)
Formula: choices^depth
Example: Permutations, n-queens, sudoku, Shrinking / 
...
Starting from index 0, the code explores every increasing subsequence 
that starts with 1. That's:
{1}, {1,2}, {1,3}, {1,4}, {1,2,3}, {1,2,4}, {1,3,4}, {1,2,3,4}
That's 2³ = 8 subsequences (each of the 3 remaining elements is either 
included or not).
...
Time = (number of unique subproblems) × (work per subproblem, excluding 
recursive calls)
...
Step 2: For one call to dfs(i), how much work does it do ignoring 
the recursive calls?
The for loop iterates from i+1 to n-1 and does O(1) per iteration 
(comparison, Math.max). Worst case: O(n).
Step 3: Multiply
n states × O(n) work per state = O(n²)

Key intuition
Without memo: you re-solve the same subproblem many times → exponential blowup.
With memo: each subproblem is solved exactly once, then cached. 
So you just count:
